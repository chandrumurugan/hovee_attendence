...
class _MyPaymentScreenState extends State<MyPaymentScreen> {
  ...
  Future<void> _getPaymentList() async {
    ...
    var response = await WebService.getMyPayments();
    if (response!.success!) {
      setState(() {
  ...
  void _filterPayments(String status) {
    ...
    setState(() {
      ...
      if (status == "All") {
        ...
        filteredList = paymentList
            .where((payment) => payment.paymentStatus == finalStatus)
            .toList();
      }
    });
  }
                                  ...
                                  Expanded(
                                    child: Column(
                                      ...
                                      children: [
                                        _buildPaymentItem(
                                          "Plan name",
                                          filteredList[index].selectedPlan.category,
                                        ),
                                        _buildPaymentItem(
                                          "Price",
                                          "₹ ${filteredList[index].selectedPlan.price}",
                                        ),
                                        _buildPaymentItem(
                                          "Payment date",
                                          DateFormat("d MMM yyyy").format(
                                            DateFormat("dd/MM/yyyy").parse(
                                              filteredList[index].purchaseDate,
                                            ),
                                          ),
                                        ),
                                        _buildPaymentItem(
                                          "Status",
                                          filteredList[index].paymentStatus == "Unpaid"
                                              ? "Failed"
                                              : filteredList[index].paymentStatus,
                                        ),
                                      ],
                                    ),
                                  ),
